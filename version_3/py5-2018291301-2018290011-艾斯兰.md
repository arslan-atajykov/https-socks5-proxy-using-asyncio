...
邮件标题：py5-2018291301-2018290011-艾斯兰

邮件附件：py5-2018291301-2018290011-艾斯兰.md（即本文件）
...
t1=time.time()
buf = await read_data(reader,100000)
tot_data = tot_data+len(buf)                     #算所读取的数据 
t2=time.time()-t1
tot_time+=t2
if(tot_time>=1 and tot_time<1.10):              #一秒
t=time.localtime()
current_time = time.strftime("%H:%M:%S",t)
#print("oneeee")
print("{}  total_data={} KB/s\n".format(current_time,round(tot_data/1000)))
current_bandwidth=tot_data/1000                #当前带宽
tot_time=0
tot_data=0
...
await update_bandwidth(us,current_bandwidth)       #把刚收到的带宽放在 mysql database 里
await write_data(writer,buf)
except Exception as err:
logging.error(err)
