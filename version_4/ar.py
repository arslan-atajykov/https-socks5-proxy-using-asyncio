...
async def response(websocket,path):
    global us_name
    global pass_word
    us_name= await websocket.recv()
    pass_word = await websocket.recv()
    if us_name and pass_word:
        loop = asyncio.get_event_loop()
        #sys.stdout.write(f"We got the message from the client:{us_name} {pass_word}")
        #flush_then_wait()
        loop.stop()


async def update_bandwidth(username,current_bandwidth):
    pool= await aiomysql.create_pool(host = '127.0.0.1', port = 3306,user = 'root',password = '13241417851',db = 'testdb')

    async with pool.acquire() as conn:
        async with conn.cursor() as cur:

                sql = "UPDATE testdb.users SET bandwidth = %s WHERE name = %s "
                val=(str(current_bandwidth),str(username))
                await cur.execute(sql,val)
                await cur.execute('commit')  #must have


    pool.close()
    await pool.wait_closed()

async def auth_database(username,password):
    global auth
    pool= await aiomysql.create_pool(host = '127.0.0.1', port = 3306,user = 'root',password = '13241417851',db = 'testdb')
    i=999
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            for row in r:
                if row[1]==str(username) and row[2]==int(password):
...
async def exchnge2(reader, writer):
    try:
        current_bandwidth =  0
        tot_data=0
        tot_time = 0
        while True:
            t1=time.time()
            buf = await read_data(reader,100000)
            tot_data = tot_data+len(buf)
            t2=time.time()-t1
            tot_time+=t2
            if(tot_time>=1 and tot_time<1.10):
                t=time.localtime()
                current_time = time.strftime("%H:%M:%S",t)
                #print("oneeee")


                sys.stdout.write("{}  receiving_bandwidth={} KB/s\n".format(current_time,round(tot_data/1000)))
                flush_then_wait()


                current_bandwidth=tot_data/1000
                tot_time=0
                tot_data=0
            ...
            #if()
            #await update_bandwidth(us,current_bandwidth)
...
async def handle_client_connection(reader, writer):
    ...
    try:
        ...
        await write_data (remote_writer,f'{host_add} {host_port} {us_name} {pass_word}\r\n'.encode())

        f_line = await remote_reader.readline()
        con_host, con_port, *_ = f_line.decode().rstrip().split()

        if(con_host=='error_auth'):

            logging.error("server says wrong username,bye bye\n\n\n")
            sys.stdout.write('error_auth')
            flush_then_wait()
        ...
        await asyncio.wait({
            asyncio.create_task(exchnge2(reader, remote_writer)),
            asyncio.create_task(exchnge2(remote_reader, writer))
        })
...
async def handle_local_connection(reader, writer):
    ...
    try:
        ...
        await auth_database(username,password)
        ...
        if auth =='error_auth':
            await write_data(writer,f'{auth} {con_port}\r\n' .encode())
            exit(1)
        ...
        await asyncio.wait({
            asyncio.create_task(exchnge2(reader, srv_writer)),
            asyncio.create_task(exchnge2(srv_reader, writer))
        })
...
if __name__ == '__main__':
    ...
    if args.remoteHost:
        start_serve =websockets.serve(response,'localhost',1239)
